...
function viewProvider(viewName) {
    return ['$stateParams', 'NgAdminConfiguration', function ($stateParams, Configuration) {
        var view, path, entity;
        path = $stateParams.path ? $stateParams.path.split('/') : null;
        entity = path && path[path.length - 1] ? path[path.length - 1] : $stateParams.entity;
        try {
            view = Configuration().getViewByEntityAndType(entity, viewName);
        } catch (e) {
...
function routing($stateProvider) {
    ...
    $stateProvider
        ...
        .state('list', {
            url: '/list?{search:json}&{page:int}&sortField&sortDir',
            params: {
                page: { value: 1, squash: true },
                search: { value: {}, squash: true },
                sortField: null,
    ...
    $stateProvider
        .state('show', {
            parent: 'main',
            url: '/:entity{path:.*}/:id?sortField&sortDir',
